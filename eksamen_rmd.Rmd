...
```{r}
reg <- lm(home_win ~ pts_home+pts_away+yds_home+yds_away+yds_home_margin+turnovers_home+turnovers_away+turnovers_home_margin, data = games)
...
#### 6.1.1 Naive Bayes
```{r}
model_nb <- train(games_train[, 3:10], games_train[, 2], method = 'naive_bayes')  #preProcess=c("center", "scale") 
predictions_nb <- predict(object = model_nb, games_test[,3:10])
...
model_svm<- train(games_train[, 3:10], games_train[, 2], method = 'svmLinear',preProcess=c("BoxCox")) #preProcess=c("center", "scale")
predictions_svm <- predict(object = model_svm, games_test[,3:10])
...
model_knn_norm <- train(games_train[, 3:10], games_train[, 2], method = 'knn',preProcess = c("range")) 
 
# Predict the labels of the test set
predictions_knn_norm <- predict(object = model_knn_norm, games_test[,3:10]) #the first 11 variables
...
#Decision tree
model_dt <- train(games_train[, 3:10], games_train[, 2], method = 'rpart') #preProcess=c("center", "scale")

predictions_dt <- predict(object = model_dt, games_test[,3:10])
...
```{r}
standings[ ,c('year')] <- list(NULL) #deleting irrelavent variables
