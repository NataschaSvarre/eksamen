...
ggplot(data = sb_winners) + 
  ...
  theme_linedraw() + 
  labs(title = "Number of Superbowls won pr team", x = "Teams", y = "Super Bowl wins") +
  theme(plot.title = element_text(hjust = 0.5))
...
reg <- glm(home_win ~ pts_home+pts_away+yds_home+yds_away+turnovers_home+turnovers_away, data = games, family = "binomial")
...
```{r echo=FALSE, results='hide',message=FALSE, warning=FALSE}
reg2 <- glm(playoffs ~ wins+loss+points_for+points_against+points_differential+margin_of_victory+strength_of_schedule+simple_rating+offensive_ranking+defensive_ranking, data = standings)
...
#Decision tree
model_dt <- train(games_train[, 3:7], games_train[, 2], method = 'rpart')
...
model_nb2 <- train(standings_train[, 2:11], standings_train[, 12], method = 'naive_bayes')
...
```{r echo=FALSE, results='hide',message=FALSE, warning=FALSE}
#Decision tree
model_dt2 <- train(standings_train[, 2:11], standings_train[, 12], method = 'rpart')
...
#### 6.3.1 Naive Bayes
```{r echo=FALSE, results='hide',message=FALSE, warning=FALSE}
model_nb3 <- train(standings_train2[, 2:11], standings_train2[, 13], method = 'naive_bayes')
...
```{r echo=FALSE, results='hide',message=FALSE, warning=FALSE}
#Decision tree
model_dt3 <- train(standings_train2[, 2:11], standings_train2[, 13], method = 'rpart')
