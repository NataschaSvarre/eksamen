...
games[ ,c('winner', 'home_team', 'away_team', 'day', 'date', 'time', 'home_team_city', 'away_team_city', 'year', 'week')] <- list(NULL)
...
games$pts_home <- ifelse(games$home_win == 1, games$pts_win, games$pts_loss)
games$pts_away <- ifelse(games$home_win == 0, games$pts_win, games$pts_loss)
games$pts_home_margin <- games$pts_home - games$pts_away
 
games$yds_home <- ifelse(games$home_win == 1, games$yds_win, games$yds_loss)
games$yds_away <- ifelse(games$home_win == 0, games$yds_win, games$yds_loss)
games$yds_home_margin <- games$yds_home - games$yds_away
...
games_train[ ,c('year', 'week', 'away_team_name', 'home_team_name', 'winners')] <- list(NULL)

games_test <- games[-games_index,]
games_test[ ,c('year', 'week', 'away_team_name', 'home_team_name', 'winners')] <- list(NULL)
...
model_svm<- train(games_train[, 1:7], games_train[, 8], method = 'svmLinear',preProcess=c("BoxCox")) #preProcess=c("center", "scale")
predictions_svm <- predict(object = model_svm, games_test[,1:7])

# Confusion matrix 
confusionMatrix(predictions_svm, games_test[,8], positive = "1")
