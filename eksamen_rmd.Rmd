...
games$home_win <- ifelse(games$winners == games$home_team_name && games$tie == 0 ,1,0)
games$home_win <- as.factor(games$home_win)
 

# Training and test dataset  
games_index <- createDataPartition(games$home_win, p=0.8, list=FALSE)
...
model_svm<- train(games_train[, 1:12], games_train[, 13], method = 'svmLinear',preProcess=c("BoxCox")) #preProcess=c("center", "scale")
predictions_svm <- predict(object = model_svm, games_test[,1:12])

# Confusion matrix 
confusionMatrix(predictions_svm, games_test[,13], positive = "1")
...
#Copied from Group 3 last year
start_time <- Sys.time()
model_svm<- train(train_data[, -77], train_data[, 77], method = 'svmLinear') predictions_svm <- predict(object = model_svm, validation_data[,-77]) end_time <- Sys.time()
end_time - start_time
